export async function loadData() {
  ...
  let dataPath = "./data/viewer_data.json";
  if (fileName.includes("min113")) {
    dataPath = "./data/viewer_data_min113_full_top30.json";
  } else if (fileName.includes("min115")) {
    dataPath = "./data/viewer_data_min115_full_top30.json";
  }
  const response = await fetch(dataPath);
  if (!response.ok) {
    throw new Error(`${dataPath} could not be loaded`);
  }
